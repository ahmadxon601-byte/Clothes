...
interface HomePageClientProps {
    ...
    initialDailyDealProducts: ApiProduct[];
    initialDailyDealEndsAt: string | null;
}
...
const PRODUCT_PLACEHOLDER_IMAGE = 'https://placehold.co/900x1100/111111/e5e7eb?text=No+Image';
...
function imageFor(product: { thumbnail: string | null } | undefined) {
...
export default function HomePageClient({
    initialProducts,
    initialDailyDealProducts,
    initialDailyDealEndsAt,
}: HomePageClientProps) {
    ...
    const [dailyDealProducts, setDailyDealProducts] = useState<ApiProduct[]>(initialDailyDealProducts);
    const [dailyDealEndsAt, setDailyDealEndsAt] = useState<string | null>(initialDailyDealEndsAt);
    ...
    useEffect(() => {
        if (initialDailyDealProducts.length === 0) {
            void loadDailyDeals();
        }
    }, [initialDailyDealProducts.length, loadDailyDeals]);
    useEffect(() => {
