...
export default function ClothingPageClient({
  ...
  useEffect(() => {
    const activeCategory = activeSubcategory || (subcategories.length === 0 ? activeParentCategory : '');
    const hasActiveFilters = Boolean(query.trim() || activeCategory || minPrice || maxPrice || minDiscount);
    ...
    triggerFetch(false);
    return () => { if (debounceRef.current) clearTimeout(debounceRef.current); };
  }, [activeParentCategory, activeSubcategory, initialProducts.length, maxPrice, minDiscount, minPrice, query, subcategories.length, triggerFetch]);
