...
export default function ApplicationsPage() {
  ...
  const requests = useMemo(() => {
    ...
    const logicalKey = (item: (typeof items)[number]) => {
      if (item.target_store_id) return `store:${item.target_store_id}`;
      const phone = item.current_store_phone || item.store_phone || '';
      const address = item.current_store_address || item.store_address || '';
      return `fallback:${item.user_email}:${phone}:${address}`;
    };
