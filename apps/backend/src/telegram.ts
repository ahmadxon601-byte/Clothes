...
if (!config.telegramBotToken) {
  throw new Error("❌ CRITICAL ERROR: TELEGRAM_BOT_TOKEN is missing. Please check your .env file or environment variables.");
}
...
async function telegramWebhook(app: FastifyInstance) {
  ...
  // Start Long Polling if in local mode
  if (config.isLocal) {
    app.log.info("🤖 Starting Telegram Bot in Long Polling mode (local)...");

    // We don't await this so it doesn't block the server startup
    bot.start({
      onStart: (botInfo) => {
