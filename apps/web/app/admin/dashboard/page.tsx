  ...
  return (
    <AdminShell>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 14, color: '#64748b' }}>Marketplace statistikasi va tezkor amallar</p>
      </div>

      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '14px 18px', color: '#dc2626', marginBottom: 24, fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
          ⚠️ {error}
        </div>
      )}
      ...
      {!stats ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16, marginBottom: 28 }}>
          {[1, 2, 3, 4].map(i => (
            <div key={i} style={{ background: '#fff', borderRadius: 16, padding: 24, height: 110, boxShadow: '0 1px 8px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
              <div style={{ background: '#f1f5f9', borderRadius: 8, height: 20, width: '60%', marginBottom: 12 }} />
              <div style={{ background: '#f8fafc', borderRadius: 8, height: 32, width: '40%' }} />
            </div>
          ))}
        </div>
      ) : (
      ...
      {/* Quick links */}
      <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 1px 8px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
          {QUICK_LINKS.map(({ href, label, icon: Icon, color }) => (
            <Link key={href} href={href} style={{
              ...
              background: `${color}08`, textDecoration: 'none', transition: 'all 0.15s',
