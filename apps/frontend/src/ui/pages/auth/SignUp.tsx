...
export function SignUp() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const data = await api.post<{ token: string }>('/api/auth/register', { name, email, password });
        ...
        <div className="min-h-screen bg-white flex flex-col p-6 max-w-md mx-auto relative pt-12">
            <div className="flex-1 flex flex-col">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="mt-8 w-full bg-gray-900 text-white font-bold py-4 rounded-full flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-[0_8px_30px_rgb(0,0,0,0.12)] disabled:opacity-60"
                    >
                        {loading ? 'Creating account…' : <><span>Sign Up</span> <ArrowRight className="w-5 h-5 text-lime-400" /></>}
                    </button>
                </form>
            </div>
            ...
            <div className="mt-auto text-center pb-8 pt-4">
                <p className="text-gray-500 text-sm font-medium">
                    ...
                    <Link to="/signin" className="font-bold text-gray-900 hover:underline">Sign In</Link>
                </p>
            </div>
        </div>
    );
