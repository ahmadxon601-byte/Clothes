...
import {
  Menu, LogOut, BarChart2, Users, ShoppingBag, Store, FileCheck,
  Sun, Moon, Layers, Briefcase, MessageSquare, AlertTriangle, Shield,
  ScrollText, Settings, Navigation, MoreHorizontal, Globe,
} from 'lucide-react';
...
const PRIMARY_NAV = [
  { path: '/dashboard', labelKey: 'nav.dashboard', icon: BarChart2 },
      ...
      <div className='flex-1 flex flex-col min-w-0 h-full relative'>
        <header className='h-16 flex-shrink-0 bg-card border-b border-border flex items-center justify-between px-4 lg:px-8 z-30'>
          <div className='flex items-center gap-4'>
            {isTablet && (
              <button onClick={() => setDrawerOpen(true)} className='p-2 rounded-xl hover:bg-pill transition-colors'>
                <Menu size={20} />
              </button>
            )}
          </div>
          ...
          <div className='flex items-center gap-3 ml-auto'>
            ...
            <div className='relative'>
              <button
                ...
                className={cn('flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-semibold border transition-all', langMenuOpen ? 'border-accent text-accent' : 'border-border text-muted hover:text-main')}
              >
                <Globe size={14} />
                {locale.toUpperCase()}
              </button>
                ...
                {langMenuOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className='absolute right-0 top-full mt-2 z-50 w-36 overflow-hidden rounded-2xl border border-border bg-card shadow-premium'
                    >
                      {(['uz', 'ru', 'en'] as const).map((lang) => (
                        <button
                          key={lang}
                          onClick={() => { setLocale(lang); setLangMenuOpen(false); }}
                          className={cn('w-full px-4 py-3 text-left text-sm font-medium transition-colors', locale === lang ? 'bg-accent text-white' : 'text-main hover:bg-pill')}
                        >
                          {t(`lang.${lang}`)}
                        </button>
                      ))}
            ...
            {/* Theme toggle */}
            <button onClick={toggleTheme} className='p-2 rounded-xl border border-border hover:bg-pill transition-colors'>
              ...
              <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', bounce: 0, duration: 0.4 }} className='fixed bottom-0 left-0 right-0 bg-card rounded-t-3xl z-50 flex flex-col max-h-[85vh]'>
                ...
                <div className='app-scrollbar overflow-y-auto p-6 flex flex-col gap-6 pb-safe'>
                  ...
                  <div className='grid grid-cols-4 gap-y-6'>
                    {SECONDARY_NAV.map((item) => (
                      <button key={item.path} onClick={() => { router.push(item.path); setMoreSheetOpen(false); }} className='flex flex-col items-center gap-2'>
