...
export default function Stores() {
  ...
  const columns = [
    ...
    { header: 'Telefon', cell: (s: Store) => s.phone || '—' },
    { header: 'Mahsulotlar', cell: (s: Store) => <span className="font-medium text-main">{s.product_count}</span> },
    {
      header: 'Status',
      cell: (s: Store) => (
        <button
          onClick={(e) => { e.stopPropagation(); toggleActive(s); }}
          className={`px-3 py-1 rounded-full text-xs font-semibold tracking-wide transition-colors ${s.is_active ? 'bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20' : 'bg-red-500/10 text-red-600 hover:bg-red-500/20'
            }`}
        >
    ...
    },
    {
      ...
      cell: (s: Store) => (
        <IconButton
          size="sm"
          variant="ghost"
          onClick={(e) => { e.stopPropagation(); deleteStore(s.id, s.name); }}
  ...
  const cardItems = stores.map(s => ({
    ...
    status: {
      label: s.is_active ? 'Faol' : 'Nofaol',
      value: s.is_active ? 'active' as const : 'rejected' as const
    },
    metadata: `Mahsulotlar: ${s.product_count} | Tel: ${s.phone || '—'}`,
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-main tracking-tight">Do'konlar</h2>
      ...
      {isMobile ? (
        <CardList items={cardItems} emptyMessage={loading ? "Yuklanmoqda..." : "Do'kon topilmadi"} />
      ) : (
        <DataTable
          data={stores}
          columns={columns}
