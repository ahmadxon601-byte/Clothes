...
export default function Users() {
  ...
  const columns = [
    { header: 'Ism', cell: (u: User) => <span className="font-medium">{u.name || '—'}</span> },
    { header: 'Email', accessorKey: 'email' as keyof User },
    { header: 'Rol', cell: (u: User) => getRoleBadge(u.role) },
    { header: 'Qo\'shilgan sana', cell: (u: User) => new Date(u.created_at).toLocaleDateString('uz-UZ') },
    {
      header: 'Amallar',
      cell: (u: User) => (
        <IconButton
  ...
  const cardItems = users.map(u => ({
    ...
    metadata: `Qo'shildi: ${new Date(u.created_at).toLocaleDateString('uz-UZ')}`,
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-main tracking-tight">Foydalanuvchilar</h2>
      ...
      {isMobile ? (
        <CardList items={cardItems} emptyMessage={loading ? "Yuklanmoqda..." : "Foydalanuvchi topilmadi"} />
      ) : (
        <DataTable
          data={users}
          columns={columns}
