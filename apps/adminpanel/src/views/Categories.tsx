...
export default function Categories() {
  ...
  const columns = [
    { header: t('products.name'), cell: (c: Category) => <span className="font-semibold">{c.name}</span> },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
        <h2 className="text-2xl font-bold text-main tracking-tight">{t('categories.title')}</h2>
        <p className="text-sm text-muted font-medium mt-1">{t('categories.subtitle')}</p>
        </div>
      ...
      <AppCard className="p-4">
        <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
          <SearchPill placeholder={t('categories.search')} value={query} onChange={(e) => setQuery(e.target.value)} containerClassName="w-full md:max-w-md" />
          <span className="inline-flex items-center gap-2 text-sm font-medium text-muted">
            <Tags size={16} />
            {t('categories.count', { count: filtered.length })}
          </span>
        </div>
      </AppCard>
